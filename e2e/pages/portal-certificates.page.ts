...
export class PortalCertificatesPage {
  ...
  constructor(page: Page) {
    this.page = page
    this.heading = page.getByRole('heading', { level: 1 }).filter({ hasText: /Certificados de residencia fiscal/ })
    this.createButton = page.getByRole('button', { name: /Nuevo certificado/i })
    this.validCount = page.getByText('Vigentes').locator('..')
    this.expiringCount = page.getByText('Proximos a vencer').locator('..')
    this.expiredCount = page.getByText('Vencidos').locator('..')
